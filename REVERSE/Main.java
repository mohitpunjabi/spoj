...
class Graph {
	...
	public Graph(int vertexCount, int edgeCount) {
		...
		for(int i = 1; i <= vertexCount; i++) vertices[i] = new Vertex(i, vertexCount + 1, false);
	...
	public String toString() {
		...
		for(int i = 1; i <= vertexCount; i++) {
			value += (i + ": ");
			ArrayList<Edge> edges = vertices[i].edges;

			for(int j = 0; j < edges.size(); j++) value += (edges.get(j).toString() + " ");
			value += "\n";
		}
	...
	class Vertex {
		...
		public FibonacciHeap.Entry<Vertex> heapEntry;
